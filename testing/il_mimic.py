...
def make_student(
    ...
    buffer = fill_buffer(trajs_path=config['TRAIN_TRAJ_PATH'], batch_size=batch_size, run_seed=run_seed,
                         traj_shift=None, buffer_size_in_trajs=buffer_size_in_trajs,
                         sampling_rate=None, strictly_batch_data=True)
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'TRAIN_TRAJ_PATH': ['volume/MIMIC/train_mimic_ventilator_0.npy',
                            'volume/MIMIC/train_mimic_ventilator_1.npy'],
    ...
    for run_seed in range(config['NUM_REPETITIONS']):
        ...
        print("Results on test environment for run %s: accuracy %.3f, auc %.3f, apr %.3f" % (
    ...
    results_mimic = pd.read_csv(
        'testing/results/' + config['ENV'] + '/' + config['ALG'] + '/trial_' +
        str(TRIAL) + '_results.csv', header=None)

    print(
        'Average results for ' + str(config['NUM_REPETITIONS']) + ' repetitions: accuracy %.3f, auc %.3f, apr %.3f' % (
        np.mean(results_mimic[1].values),
        np.mean(results_mimic[2].values),
        np.mean(results_mimic[3].values)))
